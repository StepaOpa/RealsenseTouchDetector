...
class TouchProcessor:
    ...
    def __init__(
        ...
        self.plane_normal = None  # (a, b, c)
        self.plane_d = 0.0

        # В __init__ TouchProcessor добавьте:
        ...
        self.max_touch_area: int = 15000  # Максимальная площадь касания в пикселях
    ...
    def set_intrinsics(self, intrinsics: rs.intrinsics) -> None:
        """Установка внутренних параметров камеры для 3D-реконструкции"""
        self.intrinsics = intrinsics
        self.logger.info(
            f"Интринсики установлены: {intrinsics.width}x{intrinsics.height}, fx={intrinsics.fx}, fy={intrinsics.fy}"
        )
    ...
    def calibrate_surface_height(self) -> bool:
        """
        Калибровка поверхности как 3D-плоскости в метрическом пространстве.
        Использует 3D-точки, восстановленные из (x, y, depth) с помощью интринсиков.
        Плоскость: a*X + b*Y + c*Z + d = 0, нормаль = (a, b, c)
        """
        if len(self.surface_calibration_points) < 3:
            self.logger.warning(
                "Недостаточно точек для калибровки плоскости (минимум 3)"
            )
            return False

        if self.intrinsics is None:
            self.logger.error(
                "Интринсики камеры не установлены! Невозможно восстановить 3D-координаты."
            )
            return False

        # Собираем 3D-точки
        points_3d = []
        for x, y, depth_m in self.surface_calibration_points:
            # Восстанавливаем 3D-точку в координатах камеры
            try:
                ...
                self.logger.warning(
                    f"Не удалось депроецировать точку ({x}, {y}, {depth_m}): {e}"
                )
                continue

        if len(points_3d) < 3:
            self.logger.error("Недостаточно валидных 3D-точек для калибровки")
            return False
        ...
        # Метод главных компонент (PCA): наименьшая компонента = нормаль к плоскости
        _, _, vh = np.linalg.svd(points_3d - centroid)
        normal = vh[-1, :]  # последняя строка = направление наименьшей дисперсии

        # Нормализуем нормаль
        normal = normal / np.linalg.norm(normal)

        # Уравнение плоскости: normal · (X - centroid) = 0 → normal·X + d = 0
        d = -np.dot(normal, centroid)

        # Сохраняем параметры плоскости
        self.plane_normal = normal  # (a, b, c)
        self.plane_d = d  # свободный член

        # Для совместимости с существующим API можно сохранить и старые поля (опционально)
        self.plane_A = normal[0]
        self.plane_B = normal[1]
        self.plane_C = normal[2]

        # Оценка ошибки: расстояние каждой точки до плоскости
        distances = np.abs(points_3d @ normal + d)
        self.plane_rmse = np.sqrt(np.mean(distances**2))
        self.plane_max_error = np.max(distances)

        self.logger.info(
            f"3D-плоскость калибрована: {normal[0]:.4f}·X + {normal[1]:.4f}·Y + {normal[2]:.4f}·Z + {d:.4f} = 0"
        )
        self.logger.info(f"Нормаль: {normal}, RMSE: {self.plane_rmse:.4f} м")

        if self.plane_rmse > 0.01:
            self.logger.warning("Высокая ошибка калибровки плоскости (>1 см)!")
    ...
    def _detect_touch_contours(
        ...
        Использует 3D-расстояние до калиброванной плоскости как значение высоты объекта.

        Args:
            depth_diff: Разностное изображение глубины (используется только для бинаризации)
            current_depth: Текущее изображение глубины в миллиметрах

        Returns:
            Список кортежей (x, y, area, height_above_surface) для каждого касания,
            где height_above_surface — расстояние от точки до калиброванной плоскости (в метрах).
        """
        # Пороговая обработка на основе depth_diff (для выявления потенциальных касаний)
        _, binary = cv2.threshold(
            depth_diff, self.touch_threshold, 255, cv2.THRESH_BINARY
        )
        binary = binary.astype(np.uint8)

        # Фильтрация по глубине (диапазон допустимых расстояний от камеры)
        ...
        for contour in contours:
            ...
            if not (self.min_touch_area <= area <= self.max_touch_area):
                continue
            ...
            # === ВЫЧИСЛЕНИЕ КОРРЕКТНОЙ ВЫСОТЫ НАД ПОВЕРХНОСТЬЮ ===
            height_above_surface = 0.0

            if self.plane_normal is not None and self.intrinsics is not None:
                # Берём глубину в центре контура (в мм)
                depth_at_center = current_depth[cy, cx]
                if depth_at_center > 0:
                    # Получаем расстояние до 3D-плоскости (в метрах)
                    dist = self.get_distance_to_plane(cx, cy, depth_at_center)
                    if dist is not None:
                        height_above_surface = max(
                            0.0, dist
                        )  # только положительные значения
                    else:
                        # fallback: используем среднее по области
                        mask = np.zeros(current_depth.shape, dtype=np.uint8)
                        cv2.fillPoly(mask, [contour], 255)
                        valid_depths = current_depth[mask > 0]
                        if valid_depths.size > 0 and np.any(valid_depths > 0):
                            # Средняя глубина в мм → м
                            avg_depth_mm = np.mean(valid_depths[valid_depths > 0])
                            dist = self.get_distance_to_plane(cx, cy, avg_depth_mm)
                            if dist is not None:
                                height_above_surface = max(0.0, dist)
            else:
                # Fallback: используем старое значение из depth_diff (не рекомендуется)
                mask = np.zeros(depth_diff.shape, dtype=np.uint8)
                cv2.fillPoly(mask, [contour], 255)
                mean_diff = np.mean(depth_diff[mask > 0])
                height_above_surface = float(mean_diff)

            # Отбрасываем касания, которые не выступают над поверхностью
            if height_above_surface <= 0:
                continue

            touches.append((cx, cy, int(area), height_above_surface))
    ...
    def get_distance_to_plane(self, x: int, y: int, depth_mm: float) -> Optional[float]:
        """
        Возвращает расстояние от точки (x, y, depth) до калиброванной 3D-плоскости.
        Положительное значение = точка ВЫШЕ плоскости (ближе к камере).
        """
        if self.plane_normal is None or self.intrinsics is None:
            return None

        depth_m = depth_mm * 0.001
        try:
            point_3d = rs.rs2_deproject_pixel_to_point(self.intrinsics, [x, y], depth_m)
        except:
            return None

        # Расстояние до плоскости: (a*X + b*Y + c*Z + d) / ||n||, но нормаль уже нормирована
        distance = np.dot(self.plane_normal, point_3d) + self.plane_d
        return -distance  # минус, чтобы "ближе к камере" = положительное значение
