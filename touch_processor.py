...
class TouchPoint:
    ...
    def __init__(self, x: int, y: int, depth: float, area: int, confidence: float = 1.0) -> None:
        """
...
class TouchProcessor:
    ...
    def __init__(self, cropped_width: int, cropped_height: int, target_width: int = 1920, target_height: int = 1080) -> None:
        """
        ...
        self.depth_offset_mm: float = 0.0  # Смещение глубины в миллиметрах (-100 до +200мм)
        self.depth_scale_factor: float = 1.0  # Коэффициент масштабирования глубины (0.8 до 1.5)
        self.enable_spatial_filter: bool = True  # Включить/выключить медианную фильтрацию
        self.spatial_filter_kernel: int = 5  # Размер ядра медианного фильтра (3, 5, 7, 9)
        ...
        self.last_binary_image: Optional[np.ndarray] = None  # Последнее бинарное изображение
        self.last_depth_diff: Optional[np.ndarray] = None    # Последнее разностное изображение
        
        self.logger.info(f"TouchProcessor инициализирован: {cropped_width}x{cropped_height} -> {target_width}x{target_height}")
    ...
    def set_background(self, depth_image: np.ndarray) -> None:
        ...
        if depth_image.shape != (self.cropped_height, self.cropped_width):
            self.logger.error(f"Размер изображения глубины {depth_image.shape} не соответствует ожидаемому {(self.cropped_height, self.cropped_width)}")
            return
    ...
    def update_background(self, depth_image: np.ndarray) -> None:
        ...
        self.background_depth = np.where(
            motion_mask,
            (1 - self.background_update_rate) * self.background_depth + self.background_update_rate * depth_float,
    ...
    def add_surface_calibration_point(self, x: int, y: int, depth_image: np.ndarray) -> bool:
        """
        ...
        if 0 <= x < self.cropped_width and 0 <= y < self.cropped_height:
            ...
            x2, y2 = min(self.cropped_width, x + region_size), min(self.cropped_height, y + region_size)
            ...
            if len(valid_depths) > 0:
                ...
                self.logger.info(f"Точка калибровки добавлена: ({x}, {y}) глубина={avg_depth:.3f}м")
                return True
    ...
    def _detect_touch_contours(self, depth_diff: np.ndarray, current_depth: np.ndarray) -> List[Tuple[int, int, int, float]]:
        """
        ...
        if self.surface_height is not None:
            ...
            surface_mask = np.abs(current_depth_meters - self.surface_height) <= self.surface_tolerance
        ...
        for touch in current_touches:
            ...
            for hist_touches in self.touch_history[:-1]:  # Исключаем текущий кадр
                for hist_touch in hist_touches:
                    ...
                    distance = np.sqrt((touch.x - hist_touch.x)**2 + (touch.y - hist_touch.y)**2)
    ...
    def process_frame(self, cropped_color: np.ndarray, cropped_depth: np.ndarray, 
        ...
        if cropped_depth.shape != (self.cropped_height, self.cropped_width):
            self.logger.error(f"Размер изображения глубины {cropped_depth.shape} не соответствует ожидаемому")
            return []
        ...
        for x, y, area, mean_depth in touch_contours:
            confidence = min(1.0, area / self.min_touch_area)  # Простая оценка уверенности
    ...
    def configure_detection(self, touch_threshold: Optional[float] = None, 
                          ...
                          background_update_rate: Optional[float] = None) -> None:
        """
        ...
        if background_update_rate is not None:
            ...
            self.logger.info(f"Скорость обновления фона: {self.background_update_rate:.3f}")
    ...
    def get_debug_images(self) -> Dict[str, Optional[np.ndarray]]:
        ...
        if self.last_depth_diff is not None:
            ...
            if np.max(diff_normalized) > 0:
                diff_normalized = (diff_normalized / np.max(diff_normalized) * 255).astype(np.uint8)
            else:
    ...
    def adjust_depth_perception(self, depth_image: np.ndarray, 
    ...
    def scale_depth_perception(self, depth_image: np.ndarray, scale_factor: float = None) -> np.ndarray:
        """
    ...
    def spatial_depth_filter(self, depth_image: np.ndarray, kernel_size: int = None) -> np.ndarray:
        """
    ...
    def set_depth_scale_factor(self, scale_factor: float) -> None:
        ...
        self.logger.debug(f"Коэффициент масштабирования изменен на: {self.depth_scale_factor:.3f}")
    ...
    def set_spatial_filter_enabled(self, enabled: bool) -> None:
        ...
        self.logger.debug(f"Пространственная фильтрация: {'включена' if enabled else 'выключена'}")
